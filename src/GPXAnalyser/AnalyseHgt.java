  ...
  private void setAltitude()
  {
    ...
    for(i=0; i<coordinates.size(); i++)
    {
      try
      {
         ...
         */
        lat = Math.round(Math.round(((coordinate[1] - Double.valueOf(coordinate[1]).intValue()) * OFFSET))) - 1;
        lon = Math.round(Math.round(((coordinate[3] - Double.valueOf(coordinate[3]).intValue()) * OFFSET))) - 1;
        /* calculate the position of altitude in hgt-file */ 
        pos = (lat * OFFSET * 2) + lon * 2;
        
        /* gets the latitude for the coordinate */
        altitude = hgtFile.getHgtFile().getShort(pos);
        
        /* debug information */
        System.out.printf("lat: %f \t lon: %f \t x_lat: %d y_lon: %d pos: %d \t altitude: %d\n"
                          , coordinate[1], coordinate[3], lat, lon, pos, altitude);
